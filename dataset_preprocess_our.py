...
for idx, key in tqdm(enumerate(dataset_filter)):
    ...
    temp_data.append(data['generated_text'])  # text
    temp_data.append(data['origin_id'])  # img_id
    if data['origin_label'] == 'fake':  # label
        temp_data.append(0)
    else:
        temp_data.append(1)
    ...
    dependency_of_text = get_text_dependency(data['generated_text'])
