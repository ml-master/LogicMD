...
# from component import TextEncoder, ImageEncoder, GraphConvolution, MCO
...
warnings.filterwarnings("ignore", category=UserWarning, message=".*The NVIDIA driver.*")
...
class RFND(nn.Module):
    ...
    def __init__(self, input_size=768, out_size=300, rnn=False, rnn_type='LSTM', ch=False, finetune=True,
                 ...
                 guide_head=1, norm_type='T', threshold=0.4, rate=0.2, gcnumber=1):
        ...
        self.linear_T_top_k = nn.Sequential(nn.Linear(self.instance_dim, 1), )
        self.linear_V_top_k = nn.Sequential(nn.Linear(self.instance_dim, 1), )
        self.linear_T_T_top_k = nn.Sequential(nn.Linear(self.instance_dim, 1), )
        self.linear_T_V_top_k = nn.Sequential(nn.Linear(self.instance_dim, 1), )
        self.linear_V_V_top_k = nn.Sequential(nn.Linear(self.instance_dim, 1), )
        ...
        # todo----------------------------
        nn.init.xavier_uniform_(self.clues_T)
        nn.init.xavier_uniform_(self.clues_V)
        nn.init.xavier_uniform_(self.clues_T_T)
        nn.init.xavier_uniform_(self.clues_T_V)
        nn.init.xavier_uniform_(self.clues_V_V)
        ...
        self.inter_predicates = MCO(input_size=self.instance_dim, nhead=2, dim_feedforward=2*self.instance_dim, dropout=0.2)
        ...
        self.atoms_generation = nn.Sequential(
            ...
            nn.Dropout(p=0.3),
            nn.ReLU(),
            nn.Linear(in_features=self.instance_dim * 2, out_features=1),
            # nn.Sigmoid(),
            )
    ...
    def forward(self, imgs, encoded_texts, mask_batch_T, mask_batch_TT,
                mask_batch_TV, adj_matrix, word_len,  word_spans, device):
        ...
        for gc_i in range(self.gcnumber):
            # (N,L,D)
            E_T_top_k, E_V_top_k, E_T_T_top_k, E_T_V_top_k, E_V_V_top_k = self.generate_topk_instance(
                texts=texts_0, imgs=imgs_0, mask_T=mask_batch_T, mask_TT=mask_batch_TT, mask_TV=mask_batch_TV)
            ...
            E = torch.cat([E_T_top_k, E_V_top_k, E_T_T_top_k, E_T_V_top_k, E_V_V_top_k], dim=1).contiguous().to(device).unsqueeze(2)\
            ...
            if torch.isnan(predicates_).any():
                print("yes")
            # A, N, L
            # E = E * self.answer_set
            # atoms = self.atoms_generation(torch.cat([E, predicates, E - predicates, E * predicates],  dim=-1)).squeeze().permute(2, 0, 1)
            atoms = torch.sigmoid(self.atoms_generation(
                torch.cat([E, predicates, E - predicates, E * predicates], dim=-1))).squeeze().permute(2, 0, 1)
            # N, L
            ...
            for i in range(self.guide_head):
                ...
                final_choose_score = self.sparsemax((self.guide[2*i](predicates_).squeeze())*(
                    self.guide[2*i+1](E_Image_Text).unsqueeze(2).expand(-1, predicates_.size(1), predicates_.size(2)))) * self.sparsemax(instance_score_by_ans)
                # final_choose_score = (self.sparsemax((self.guide[2 * i](predicates_).squeeze())))
                final_choose_score = final_choose_score.permute(2, 0, 1)
                self.threshold = 0.5
                # ------------------------------------------------------------------------------
                # (A,N,L)
                ...
                # ansnumber*N
                logic_score = self.conjunction_logic_reasoning(logic_score, device)
                # H, N
            ...
            if gc_i != len(self.gc):
                graph = torch.cat([texts_0, imgs_0], dim=1).to(device)
    ...
    def generate_topk_instance(self, texts, imgs, mask_T, mask_TT, mask_TV):
        ...
        E_T_score = self.linear_T_top_k(E_T).squeeze().masked_fill_(mask_T, 1e-9)
        ...
        E_T_T_score = self.linear_T_T_top_k(E_T_T).squeeze().masked_fill_(mask_TT, 1e-9)
        E_T_V_score = self.linear_T_V_top_k(E_T_V).squeeze().masked_fill_(mask_TV, 1e-9)
        # may add mask afterwards
    ...
    def bind_relation_instance(self, E_top_k, clues):
        ...
        E_top_k_1 = E_top_k.unsqueeze(2).expand(-1, -1, self.ans_number, -1)
        #  (N, L, A，self.instance_dim)
    ...
    def conjunction_logic_reasoning(self, logic_score, device):
        """
        ...
        if self.norm_type == 'minimum':
            ...
            for sample in logic_score:
                ...
                logic_score_.append(torch.tensor(tmp).to(device))
        else:
